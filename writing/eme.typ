...
- the PL spreads out with increasing $z$, leading to more regions of empty space/where we don't care about the fields (because we damp them at the port edges) but where we're forced to compute them anyway;
...
- we can't make use of the linearity or the independence of the ports.

I found that the preferred method for waveguides such as optical fibers and MMIs, where the size along the direction of propagation is much larger than that perpendicular to it, is _eigenmode expansion_ (EME). This involves expressing the electric field as a linear combination of eigenmodes, and propagating thoe coefficients of those eigenmodes through the waveguide. This has the advantage of being an exact solution -- the number of components is just the number of eigenmodes, which is limited by the size of the input port relative to the wavelength of light. Further, the method's integration error is limited by the cell resolution along the direction of propagation, and we can treat each port the same as it has the same index of refraction profile and is unaffected by other ports. 
