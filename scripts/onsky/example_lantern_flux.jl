...
A = make_unitary_matrix(18)
y_all, ymin, ymax = @chain abs2.(A * X_all) convert(Matrix{Float32}, _) zero_one_ify
...
optim = Flux.setup(Adam(1e-2), model)
...
begin
    a = Animation()
    @showprogress for i = 1:20:(n-n_split)
        frame(a, plot_phase_screens(i))
    end
    gif(a, "figures/example_nn.gif", fps=2)
end
...
begin
    for i in [argmax(rms_resids), argmin(rms_resids)]
        ...
        Plots.savefig("figures/example_nn_test_$i.pdf")
    end
    p
end